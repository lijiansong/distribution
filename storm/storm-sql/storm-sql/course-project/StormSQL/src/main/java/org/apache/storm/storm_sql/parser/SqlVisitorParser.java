...
public class SqlVisitorParser extends sqlBaseVisitor<String>{
    ...
    ArrayList<ArrayList<String>> select_list=new ArrayList<>();
    ...
    public ArrayList<ArrayList<String>> getSelect_list() {
        return select_list;
    }
...
//            }
            if(spilit.length!=0){
                ...
                if(spilit[0].contains("(")){
                    ...
                    tmp.add(spilit[0]);tmp.add(spilit[1]);
                    select_list.add(tmp);
                }
            }
    ...
    //expression->ID
    @Override
    public String visitId(sqlParser.IdContext ctx) {
